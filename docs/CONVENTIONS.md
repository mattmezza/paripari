...
- **Auth**: email + password (bcrypt via `golang.org/x/crypto/bcrypt`), secure HTTP-only cookie sessions stored in DB (`sessions` table). Sessions honour a per-user lifetime (7/30/90/365 days or never) and slide forward on activity past the half-life; a NULL `expires_at` never expires. TOTP two-factor (Google Authenticator, `github.com/pquerna/otp`) with 8 single-use hashed recovery codes; sensitive actions (password change, 2FA changes, data export) require a step-up re-auth valid for 10 minutes (`sessions.verified_at`). CSRF: same-site strict cookies + custom header check for htmx POSTs (htmx sends `HX-Request`; also embed a csrf token input in forms). No passkeys in v1.
